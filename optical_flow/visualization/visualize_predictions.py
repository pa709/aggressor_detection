...
# ──────────────────────────────────────────────────────────────────────────────
# Hyperparameters  (match train_tcn.py)
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# TCN model — numpy implementation (identical to train_tcn.py)
# ──────────────────────────────────────────────────────────────────────────────
...
def conv1d_forward(x, W, b, dilation, padding):
    ...
    for k_idx in range(K):
        ...
        out += np.einsum("bct,oc->bot", x_pad[:, :, offset: offset + T_out], W[:, :, k_idx])
...
def conv1d_backward(grad_out, x_pad, W, dilation, padding):
    ...
    for k_idx in range(K):
        ...
        dW[:, :, k_idx] = np.einsum("bot,bct->oc", grad_out, x_pad[:, :, sl])
        dx_pad[:, :, sl] += np.einsum("bot,oc->bct", grad_out, W[:, :, k_idx])
...
def dropout_backward(g, mask, rate=0): return g * mask if mask is not None else g
...
def init_params():
    ...
    return {
        "W1": he((16, 11, 3)), "b1": np.zeros(16, dtype=np.float32),
...
def train_model(X_all, y_all):
    """Train TCN on full dataset. Returns (params, mu, std)."""
    np.random.seed(SEED)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        # Use training loss for early stopping (no val set — full dataset)
...
def predict_tracks(X, params, mu, std):
    """
    X: (N, 11, 30)
    Returns int array of 0/1  (1 = predicted aggressor)
    """
...
def get_box_in_frame(tracks, track_id_str, frame_num):
    """
    Returns (xtl, ytl, xbr, ybr) or None.
    Handles merged track IDs like '1+2' by checking each component.
    """
    tids = [t.strip() for t in track_id_str.split("+")]
    for tid in tids:
        if tid not in tracks:
            continue
        if frame_num in tracks[tid]["frames"]:
            box = tracks[tid]["frames"][frame_num]
            if box[4] == 0:   # not outside
...
def draw_box(frame, xtl, ytl, xbr, ybr, color, label_text):
    ...
    cv2.rectangle(frame,
                  ...
                  (x1 + tw + 4, label_y + 2),
                  color, -1)
    # Text (black for readability on both red and yellow)
    cv2.putText(frame, label_text,
                (x1 + 2, label_y - baseline),
                FONT, FONT_SCALE, (0, 0, 0), THICKNESS - 1, cv2.LINE_AA)


def render_video(video_path, tracks, track_predictions, ws, we, out_path):
    """
    track_predictions: {track_id_str: int}  1=aggressor, 0=non-aggressor
    ws, we: window start and end frame (inclusive)
    """
    ...
    for fn in range(ws, we + 1):
        ...
        for tid_str, pred in track_predictions.items():
            box = get_box_in_frame(tracks, tid_str, fn)
            if box is None:
                continue
            xtl, ytl, xbr, ybr = box
            if pred == 1:
...
def load_all_npz(npz_files):
    ...
    Returns:
        X_all  : (N, 11, 30)
        y_all  : (N,)  int  1=aggressor 0=non-aggressor
        records: list of {video_name, track_id, label_str, features (11,30)}
    """
    records = []
    for ann, path in npz_files.items():
        d    = np.load(path, allow_pickle=True)
        feat = d["features"].astype(np.float32)   # (N, 30, 11)
        feat = feat.transpose(0, 2, 1)             # → (N, 11, 30)
        lbls = d["labels"]
        vids = d["video_names"]
        tids = d["track_ids"]
        for i in range(len(lbls)):
            records.append({
                ...
                "track_id"   : str(tids[i]),
                "label_str"  : str(lbls[i]),
                "features"   : feat[i],
            })

    X_all = np.stack([r["features"] for r in records])
    y_all = np.array([1 if r["label_str"] == "aggressor" else 0
...
def main():
    parser = argparse.ArgumentParser(
        description="Visualize non-pairwise TCN aggressor predictions on video."
    )
    parser.add_argument("--npz", nargs="+", required=True,
                        metavar="ANNOTATOR:PATH",
                        help="NPZ files: --npz annotator1:/path/annotator1.npz ...")
    ...
    print(f"  {len(records)} tracks loaded  "
          f"(aggressor={int(y_all.sum())}  non-aggressor={int((y_all==0).sum())})")

    # ── Train model ──────────────────────────────────────────────────────────
    print("\nTraining TCN on full dataset ...")
    params, mu, std = train_model(X_all, y_all)

    # ── Inference ────────────────────────────────────────────────────────────
    print("\nRunning inference ...")
    preds = predict_tracks(X_all, params, mu, std)
    ...
    for r in records:
        vn = r["video_name"]
        if vn not in videos_to_render:
            videos_to_render[vn] = []
        videos_to_render[vn].append(r)

    # ── Render ───────────────────────────────────────────────────────────────
    print(f"\nRendering {len(videos_to_render)} videos ...")
    rendered = 0
    skipped  = 0

    for video_name, track_records in sorted(videos_to_render.items()):
        print(f"\n  {video_name}")
        ...
        # Parse XML → get tracks and first aggressor frame
        try:
        ...
        # Build prediction dict for this video
        track_predictions = {}
        for r in track_records:
            track_predictions[r["track_id"]] = r["pred"]
            gt   = "aggressor" if r["label_str"] == "aggressor" else "non-aggressor"
            pred = "aggressor" if r["pred"] == 1 else "non-aggressor"
            match = "✓" if gt == pred else "✗"
            print(f"    Track {r['track_id']:>4s}  GT={gt:<14s}  Pred={pred:<14s}  {match}")

        # Render
        out_name = f"{video_name}_predicted.mp4"
        out_path = os.path.join(args.out_dir, out_name)
        render_video(video_path, tracks, track_predictions, ws, we, out_path)
